...
class Stm_Services_Widget_SM extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if( $services ){
			...
			foreach( $services as $service ){
				$output .= '<li' . (( get_the_ID() == $service->ID ) ? ' class="active"' : '' ) . '>' . get_the_title( $service->ID ) . '</li>';
			}
			$output .= '</ul>';
			echo $output;
		}
...
function register_SM_services_widget() {
	register_widget( 'Stm_Services_Widget_SM' );
}
add_action( 'widgets_init', 'register_SM_services_widget' );